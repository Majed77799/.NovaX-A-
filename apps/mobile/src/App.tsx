...
export default function App() {
	...
	async function send(text: string) {
		...
		try {
			...
			const base = process.env.EXPO_PUBLIC_API_URL ?? process.env.NEXT_PUBLIC_APP_URL ?? 'http://localhost:3000';
			const res = await fetch(`${base}/api/chat`, { method: 'POST', headers: { 'Content-Type': 'application/json', ...(token ? { Authorization: `Bearer ${token}` } : {}) }, body: JSON.stringify({ messages: [...messages, userMsg] }) });
	...
	return (
		<LinearGradient colors={["#F6E7FF","#E9F0FF","#D7F7FF"]} start={{x:0,y:0}} end={{x:1,y:1}} style={{ flex: 1 }}>
			<SafeAreaView style={{ flex: 1 }}>
				<View style={{ height: 96, width: 96, borderRadius: 48, margin: 12, shadowColor: '#7841FF', shadowOpacity: 0.35, shadowRadius: 16, backgroundColor: 'rgba(255,255,255,0.35)', overflow: 'hidden' }} />
				...
				<View style={{ position: 'absolute', left: 0, right: 0, bottom: 0, padding: 16 }}>
					<View style={{ backgroundColor: '#fff', borderRadius: 999, borderWidth: 1, borderColor: 'rgba(15,18,35,0.08)', padding: 10, flexDirection: 'row', alignItems: 'flex-end', gap: 8, shadowColor: '#101828', shadowOpacity: 0.06, shadowRadius: 20 }}>
						...
						<TextInput style={{ flex: 1, paddingHorizontal: 12, paddingVertical: 8, maxHeight: 24*4, fontFamily: fontLoaded ? 'Urbanist' : undefined }} value={input} multiline onChangeText={setInput} onSubmitEditing={() => send(input)} placeholder="Message Ello" />
